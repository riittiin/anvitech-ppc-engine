...
- `api._current_masters()`: after parsing the workbook → `table = book_store.load_operator_table()`; if None and workbook has operators → seed (`seed_rows_from_masters`, `week_anchor=last_friday(today)`, save); if table → `table, n = rotate_table(table, date.today())`; persist when n>0; `masters.operators = to_operators(table["operators"])`. Cache: the `_MASTERS_CACHE` must include the operator-table state (add the table's json to the cache key or re-apply on every call — re-apply overlay on every `_current_masters()` call, cache only the parsed workbook; measure nothing — overlay is cheap).
