...
def _build_contended_operator_workbook() -> bytes:
    """A minimal, self-contained workbook (same sheet names/layout as
    tests/new_sample_workbook.py, so both the old and new loaders parse it natively)
    with a DELIBERATELY contended operator pool: two "First shift" operators both
    qualified for CNC1, one narrow (only CNC1) and one flexible (CNC1, CNC2). This
    is the minimal case where "scarce" (least-flexible-first) and "flexible"
    (most-flexible-first) must pick different people for the same machine+shift —
    unlike tests/new_sample_workbook.py, where every (machine, shift) pool has at
    most one qualified candidate (see task-3-report.md for that finding). Kept
    local to this test so no shared fixture used by other tests is touched.
    """
    import openpyxl
    ...
    ws.append(["CNC lathe", "CNC1", 250, 19.5])
    ws.append(["CNC lathe", "CNC2", 250, 19.5])

    # Operator & shift Master — both operators on "First shift", differing only in
    # how many machines they're qualified for (the thing operator_pick sorts on).
    ws = wb.create_sheet("Operator & shift Master")
    ws.append(["Operator Name", "Role", "Preferred Machines", "Shift", None,
               "Shift", "Start", "End", "Notes"])
    ws.append(["Narrow", "operator", "CNC1", "First shift", None,
               "First shift", "08:00", "19:00", ""])
    ws.append(["Flex", "operator", "CNC1, CNC2", "First shift", None,
    ...
    _set(ws, 2, hdr)
    item_code = "CONTEND-A-01"
    _set(ws, 3, {0: 1, 1: "ACME", 2: "CONTENDED PART", 3: item_code, 6: "Dia 20 SS", 10: 50,
                 12: "CNC FIRST SIDE", 13: 3, 14: 3, 15: "CNC1", 16: "CNC1",
                 17: "DISPATCH", 18: 0, 19: 0})
    ...
    _set(ws, 2, {5: "CSO-001", 8: "ACME", 19: item_code, 20: "CONTENDED PART",
                 21: 50, 23: date(2025, 3, 20), 24: "", 27: 50})
...
def test_operator_pick_changes_the_live_new_engine_plan():
    """Behavioural proof the policy reaches the real scheduler (not just a field
    copy). tests/new_sample_workbook.py turned out to have NO (machine, shift) with
    more than one qualified candidate, so scarce/flexible are trivially identical
    on it (see task-3-report.md) — this test instead builds its own minimal
    CONTENDED workbook (two "First shift" operators qualified for CNC1, one narrow
    one flexible) so operator_pick has an actual choice to make, and drives the
    real path: both loaders -> rule1_consolidate -> _orders_from_batches ->
    ppc_engine.scheduler.decode(..., _plan_config(cfg)). Verifies wiring in code,
    per the standing 'test behaviour before explaining it' lesson."""
    from dataclasses import replace as _replace

    wb_bytes = _build_contended_operator_workbook()
    so_lines, _old_masters = loaders.load_all(io.BytesIO(wb_bytes))
    new_masters = new_load(io.BytesIO(wb_bytes)).masters

    cfg = Config(scheduler="new", plan_start_date=date(2025, 3, 3), apply_operator_logic=True)
    batches = rule1_consolidate.run(so_lines, cfg)
    orders, _ = _orders_from_batches(batches, new_masters)
    seq = [(b.batch_id, b.item_code) for b in batches]

    def assignments(pick):
        run_cfg = _replace(cfg, operator_pick=pick)
        sched = decode(orders, seq, new_masters, _plan_config(run_cfg))
    ...
    flexible = assignments("flexible")
    assert scarce != flexible, "operator_pick had no effect on the plan"

    cnc1_keys = [k for k in scarce if k[2] == "CNC1"]
    assert cnc1_keys, "expected a CNC1 assignment in the contended plan"
    assert all(scarce[k] == "Narrow" for k in cnc1_keys), \
        f"scarce should pick the least-flexible operator (Narrow) on CNC1: {scarce}"
    assert all(flexible[k] == "Flex" for k in cnc1_keys), \
        f"flexible should pick the most-flexible operator (Flex) on CNC1: {flexible}"
